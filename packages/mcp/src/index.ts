...
import {
  createVectorDatabaseInstance,
  logVectorDatabaseProviderInfo,
} from "./vectordb.js";
import { createRerankerInstance, logRerankerInfo } from "./reranker.js";
...
class ContextMcpServer {
  ...
  constructor(config: ContextMcpConfig) {
    ...
    const vectorDatabase = createVectorDatabaseInstance(config);
    logVectorDatabaseProviderInfo(config, vectorDatabase);

    // Initialize reranker (optional)
    const reranker = createRerankerInstance(config);
    logRerankerInfo(config, reranker);

    // Initialize sparse embedding for hybrid search (optional)
    let sparseEmbedding: SparseEmbedding | undefined;
    if (config.sparseEmbeddingEnabled) {
      console.log(
        `[SPARSE] Initializing BM25 sparse embedding for hybrid search...`,
      );
      sparseEmbedding = createCodeSparseEmbedding();

      // Apply custom BM25 parameters if provided
      if (
        config.sparseEmbeddingK1 !== undefined ||
        config.sparseEmbeddingB !== undefined
      ) {
        sparseEmbedding.updateConfig({
          k1: config.sparseEmbeddingK1,
          b: config.sparseEmbeddingB,
        });
        console.log(
          `[SPARSE] BM25 parameters: k1=${config.sparseEmbeddingK1 ?? 1.5}, b=${config.sparseEmbeddingB ?? 0.75}`,
        );
      }
      console.log(
        `[SPARSE] ✅ Sparse embedding initialized: ${sparseEmbedding.getProvider()}`,
      );
    } else {
      console.log(
        `[SPARSE] Sparse embedding disabled (set SPARSE_EMBEDDING_ENABLED=true to enable)`,
      );
    }
