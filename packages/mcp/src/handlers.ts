...
export class ToolHandlers {
  ...
  private async syncIndexedCodebasesFromCloud(): Promise<void> {
    try {
      ...
      // Skip cloud sync for non-Milvus databases (Qdrant, etc.)
      // This sync is designed for Zilliz Cloud only
      const dbName = vectorDb.constructor.name;
      if (dbName === "QdrantVectorDatabase") {
        console.log(
          `[SYNC-CLOUD] ⏭️ Skipping cloud sync for ${dbName} (local database)`,
        );
        return;
      }
      ...
      console.log(
        `[SYNC-CLOUD] 📋 Found ${collections.length} collections in Zilliz Cloud`,
      );
