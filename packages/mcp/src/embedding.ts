  ...
  switch (config.embeddingProvider) {
    ...
    case "Fireworks":
      if (!config.fireworksApiKey) {
        console.error(
          `[EMBEDDING] ❌ Fireworks API key is required but not provided`,
        );
        throw new Error(
          "FIREWORKS_API_KEY is required for Fireworks embedding provider",
        );
      }
      console.log(
        `[EMBEDDING] 🔧 Configuring Fireworks with model: ${config.embeddingModel}`,
      );
      const fireworksEmbedding = new FireworksEmbedding({
        apiKey: config.fireworksApiKey,
        model: config.embeddingModel,
      });
      console.log(
        `[EMBEDDING] ✅ Fireworks embedding instance created successfully`,
      );
...
export function logEmbeddingProviderInfo(
  ...
  switch (config.embeddingProvider) {
    ...
    case "Fireworks":
      console.log(
        `[EMBEDDING] Fireworks configuration - API Key: ${config.fireworksApiKey ? "✅ Provided" : "❌ Missing"}, Model: ${config.embeddingModel}`,
      );
      break;
  }
}
