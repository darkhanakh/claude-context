...
/**
 * Sparse vector representation for BM25/SPLADE-style embeddings
 * Contains parallel arrays of token indices and their weights
 */
export interface SparseVector {
  indices: number[]; // Token IDs (must be integers)
  values: number[]; // Importance weights (floats)
}

export interface VectorDocument {
  id: string;
  vector: number[];
  sparseVector?: SparseVector; // Optional sparse vector for hybrid search
  content: string;
  relativePath: string;
  startLine: number;
  endLine: number;
...
export interface HybridSearchRequest {
  data: number[] | SparseVector | string; // Query vector (dense), sparse vector, or text
  anns_field: string; // Vector field name (vector, sparse_vector, dense, sparse)
