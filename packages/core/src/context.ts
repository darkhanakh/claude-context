...
import { Reranker, RerankDocument } from "./reranker";
...
export class Context {
  ...
  constructor(config: ContextConfig = {}) {
    ...
    if (this.reranker) {
      console.log(
        `[Context] 🔄 Reranker enabled: ${this.reranker.getProvider()} - ${this.reranker.getModel()}`,
      );
    }

    // Initialize optional sparse embedding for hybrid search
    this.sparseEmbedding = config.sparseEmbedding;
    if (this.sparseEmbedding) {
      console.log(
        `[Context] 📊 Sparse embedding enabled: ${this.sparseEmbedding.getProvider()}`,
      );
    }
  ...
  /**
   * Get sparse embedding instance (if configured)
   */
  getSparseEmbedding(): SparseEmbedding | undefined {
  ...
  async semanticSearch(
    ...
    if (isHybrid === true) {
      ...
      console.log(
        `[Context] ✅ Generated dense embedding vector with dimension: ${queryEmbedding.vector.length}`,
      );

      // 2. Generate sparse query vector if sparse embedding is available
      let querySparseVector: { indices: number[]; values: number[] } | null =
        null;
      if (this.sparseEmbedding && this.sparseEmbedding.isReady()) {
        querySparseVector = this.sparseEmbedding.embedQuery(query);
        console.log(
          `[Context] ✅ Generated sparse query vector with ${querySparseVector.indices.length} non-zero terms`,
        );
      }

      // 3. Prepare hybrid search requests
      ...
      // Add sparse search request if sparse vector is available
      if (querySparseVector && querySparseVector.indices.length > 0) {
        ...
        console.log(
          `[Context] 🔍 Search request 2 (sparse): anns_field="sparse", non-zero terms=${querySparseVector.indices.length}, limit=${topK}`,
        );
      } else {
        console.log(
          `[Context] ⚠️ Sparse search skipped (no sparse embedding or empty query vector)`,
        );
      }

      console.log(
        `[Context] 🔍 Search request 1 (dense): anns_field="dense", vector_dim=${queryEmbedding.vector.length}, limit=${topK}`,
      );
      ...
      let results: SemanticSearchResult[] = searchResults.map((result) => ({
      ...
      // 5. Apply external reranker if available (e.g., Fireworks Qwen3 Reranker)
      if (this.reranker && results.length > 0) {
        console.log(
          `[Context] 🔄 Applying ${this.reranker.getProvider()} reranker (${this.reranker.getModel()}) to ${results.length} results...`,
        );

        const rerankDocuments: RerankDocument[] = results.map((r, idx) => ({
        ...
        try {
          ...
          // Map reranked results back to SemanticSearchResult format
          results = rerankedResults.map((rr) => ({
            content: rr.document.content,
            relativePath: rr.document.metadata?.relativePath || "",
            startLine: rr.document.metadata?.startLine || 0,
            endLine: rr.document.metadata?.endLine || 0,
            language: rr.document.metadata?.language || "unknown",
            score: rr.relevanceScore, // Use reranker's relevance score
          }));

          console.log(
            `[Context] ✅ Reranking complete. Top result score: ${results[0]?.score.toFixed(4) || "N/A"}`,
          );
        } catch (error) {
          console.error(
            `[Context] ⚠️ Reranking failed, using original results:`,
            error,
          );
      ...
      // 4. Apply external reranker if available (e.g., Fireworks Qwen3 Reranker)
      if (this.reranker && results.length > 0) {
        console.log(
          `[Context] 🔄 Applying ${this.reranker.getProvider()} reranker (${this.reranker.getModel()}) to ${results.length} results...`,
        );
  ...
  private async processFileList(
    ...
    // Pre-pass: Build sparse vocabulary if sparse embedding is enabled
    if (this.sparseEmbedding && isHybrid === true) {
      console.log(
        `[Context] 📊 Building sparse vocabulary from ${filePaths.length} files (pre-pass)...`,
      );
      const allChunkContents: string[] = [];
      ...
      for (const filePath of filePaths) {
        try {
          ...
          for (const chunk of chunks) {
            allChunkContents.push(chunk.content);
            // Limit vocabulary building to avoid memory issues
            if (allChunkContents.length >= CHUNK_LIMIT) {
              break;
            }
          }
        } catch {
          // Skip files that can't be read during pre-pass
        }
        if (allChunkContents.length >= CHUNK_LIMIT) {
          break;
        }
      ...
      this.sparseEmbedding.buildVocabulary(allChunkContents);
      console.log(
        `[Context] ✅ Sparse vocabulary built with ${this.sparseEmbedding.getVocabularySize()} terms from ${allChunkContents.length} chunks`,
      );
    }
  ...
  private async processChunkBatch(
    ...
    // Generate sparse embeddings if sparse embedding is configured and ready
    let sparseEmbeddings: Array<{ indices: number[]; values: number[] }> = [];
    if (
      this.sparseEmbedding &&
      isHybrid === true &&
      this.sparseEmbedding.isReady()
    ) {
      // Generate sparse embeddings for all chunks
      sparseEmbeddings = this.sparseEmbedding.embedBatch(chunkContents);
    }
    ...
    if (isHybrid === true) {
      ...
      const documents: VectorDocument[] = chunks.map((chunk, index) => {
        ...
        if (sparseEmbeddings.length > 0 && sparseEmbeddings[index]) {
          doc.sparseVector = sparseEmbeddings[index];
        }
